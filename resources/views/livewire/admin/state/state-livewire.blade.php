<div>
    <div class="mx-auto pb-12 p-6 sm:px-6 lg:px-8 bg-white dark:bg-zinc-800 overflow-hidden shadow-sm sm:rounded-lg text-zinc-900 dark:text-zinc-100">
        ...
        {{-- create --}}
        @if($add_new == true)
            @include('livewire.admin.state.includes.create')
        @endif

        {{-- search --}}
        @include('livewire.admin.state.includes.search')

        {{-- card --}}
        @foreach ($states as $state)
            @include('livewire.admin.state.includes.card')
        @endforeach
        @include('livewire.admin.state.includes.modal-update')
        @include('livewire.admin.state.includes.modal-delete')
