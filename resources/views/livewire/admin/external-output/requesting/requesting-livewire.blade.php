<div>
    <div class="mx-auto pb-12 p-6 sm:px-6 lg:px-8 bg-white dark:bg-zinc-800 overflow-hidden shadow-sm sm:rounded-lg text-zinc-900 dark:text-zinc-100">
        ...
        <div class="flex mb-4">
            <h2 class="font-semibold text-lg text-zinc-800 dark:text-zinc-400 mb-5">Cadastro de Requisitante</h2>
        </div>
        ...
        {{-- create --}}
        @if($add_new == true)
            @include('livewire.admin.external-output.requesting.includes.create')
        @endif

        {{-- search --}}
        @include('livewire.admin.external-output.requesting.includes.search')

        {{-- card --}}
        @foreach ($requestings as $requesting)
            @include('livewire.admin.external-output.requesting.includes.card')
        @endforeach
        @include('livewire.admin.external-output.requesting.includes.modal-update')
        @include('livewire.admin.external-output.requesting.includes.modal-delete')

        {{-- paginação --}}
        <div class="pl-2 py-4 mt-4 text-zinc-50 dark:text-zinc-400 border-t border-blue-300 dark:border-blue-500 pb-3">
            {{ $requestings->onEachSide(1)->links() }}
        </div>
    </div>
</div>