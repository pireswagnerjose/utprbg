<div>
    <div class="mx-auto pb-12 p-6 sm:px-6 lg:px-8 bg-white dark:bg-zinc-800 overflow-hidden shadow-sm sm:rounded-lg text-zinc-900 dark:text-zinc-100">
        ...
        <div class="flex mb-4">
            <h2 class="font-semibold text-lg text-zinc-800 dark:text-zinc-400 mb-5">Modalidade do Atendimento</h2>
        </div>
        ...
        {{-- create --}}
        @if($add_new == true)
            @include('livewire.admin.legal-assistance.modality-care.includes.create')
        @endif

        {{-- search --}}
        @include('livewire.admin.legal-assistance.modality-care.includes.search')

        {{-- card --}}
        @foreach ($modality_cares as $modality_care)
            @include('livewire.admin.legal-assistance.modality-care.includes.card')
        @endforeach
        @include('livewire.admin.legal-assistance.modality-care.includes.modal-update')
        @include('livewire.admin.legal-assistance.modality-care.includes.modal-delete')

        {{-- paginação --}}
        <div class="pl-2 py-4 mt-4 text-zinc-50 dark:text-zinc-400 border-t border-blue-300 dark:border-blue-500 pb-3">
            {{ $modality_cares->onEachSide(1)->links() }}
        </div>
    </div>
</div>