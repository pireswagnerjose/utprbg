<div>
    <div class="mx-auto pb-12 p-6 sm:px-6 lg:px-8 bg-white dark:bg-zinc-800 overflow-hidden shadow-sm sm:rounded-lg text-zinc-900 dark:text-zinc-100">
        ...
        {{-- create --}}
        @if($add_new == true)
            @include('livewire.admin.pad.pad-local.includes.create')
        @endif

        {{-- search --}}
        @include('livewire.admin.pad.pad-local.includes.search')

        {{-- card --}}
        @foreach ($pad_locals as $pad_local)
            @include('livewire.admin.pad.pad-local.includes.card')
        @endforeach
        @include('livewire.admin.pad.pad-local.includes.modal-update')
        @include('livewire.admin.pad.pad-local.includes.modal-delete')

        {{-- paginação --}}
        <div class="pl-2 py-4 mt-4 text-zinc-50 dark:text-zinc-400 border-t border-blue-300 dark:border-blue-500 pb-3">
            {{ $pad_locals->onEachSide(1)->links() }}
        </div>
    </div>
</div>