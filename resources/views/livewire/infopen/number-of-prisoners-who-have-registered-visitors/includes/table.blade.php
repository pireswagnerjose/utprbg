<table class="w-full text-sm text-left rtl:text-right text-zinc-500 dark:text-zinc-400">
    <thead class="text-xs text-zinc-700 uppercase bg-zinc-50 dark:bg-zinc-700 dark:text-zinc-400">
        <tr>
            ...
            <th scope="col" class="p-2"> Avatar </th>
    ...
    <tbody>
        @forelse ($prisoners as $key=>$prisoner)
            <tr
                ...
                <td class="p-2">
                    <a title='{{ $prisoner->name }}' href='{{ asset("storage/$prisoner->photo") }}'
                        rel='shadowbox[galeria]'>
                        <img class="object-cover w-10 h-10 rounded-full" src='{{ asset("storage/$prisoner->photo") }}'
                            alt="{{ $prisoner->name }}" />
                    </a>
                </td>
                <td class="p-2">
