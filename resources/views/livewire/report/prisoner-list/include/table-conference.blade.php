...
<table class="w-full text-sm text-left rtl:text-right text-zinc-500 dark:text-zinc-400">
    ...
    <tbody>
        ...
        <tr
            ...
            @endif
            <td class="p-2"> <a class="text-blue-700" href="{{ route('prisoners.show', $data->prisoner->id ) }}"> {{
                    $data->prisoner->name }} </a> </td>
            <td class="p-2 text-center text-red-600">{{ empty($data->cell->cell) ? null : $data->cell->cell }}</td>
            @php $prison = $prisons->where('prisoner_id', $data->prisoner->id)->first() @endphp
            <td class="p-2 text-center"> {{ empty($prison->entry_date) ? null :
                \Carbon\Carbon::parse($prison->entry_date)->format('d/m/Y') }}</td>
        </tr>
        @empty
        <td class="p-2 text-center"> Não existe resultado para essa consulta. </td>
        @endforelse
    </tbody>
</table>
@endforeach