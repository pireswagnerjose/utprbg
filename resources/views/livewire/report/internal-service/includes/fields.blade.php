{{-- Linha 1 --}}
<div class="grid grid-cols-5 gap-6 mt-624">
  <div class="relative col-span-2 z-0 w-full group">
      <select wire:model.live.debounce.500ms="type_service_id" class="uppercase block py-1 mt-1 px-0 w-full text-sm text-zinc-500 dark:text-zinc-400 bg-transparent border-0 border-b border-zinc-400 dark:border-zinc-600 dark:focus:border-blue-500 focus:border-blue-600 appearance-none focus:outline-none focus:ring-0 peer">
         <option class="text-zinc-900 dark:text-zinc-600" selected value="">TIPO DO ATENDIMENTO</option>
            @foreach ($type_services as $type_service)
         <option class="text-zinc-900 dark:text-zinc-600" value="{{ $type_service->id }}">{{ $type_service->type_service }}</option>
         @endforeach
      </select>
   </div>
