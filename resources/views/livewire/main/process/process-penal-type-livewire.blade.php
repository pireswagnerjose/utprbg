<div>
    @can('create_penal_type_process')
    <button wire:click="addNew"><span class="text-blue-600 mr-4">[ Adicionar ]</span></button>
    @endcan
    {{-- create --}}
    @if($add_new == true)
    <div id="search-box"
        class="flex flex-col items-center px-2 my-4 justify-center w-full border-b border-blue-300 dark:border-blue-500 pb-3">
        <div class="flex justify-center items-center w-full">
            ...
            <input wire:model.live.debounce.500ms='search' type="text" placeholder="Search..."
                class="w-2/6 text-zinc-600 dark:text-zinc-300 bg-zinc-300 hover:bg-zinc-200 dark:bg-zinc-600 dark:hover:bg-zinc-500 ml-2 border-none rounded px-4 py-2" />
        </div>
    </div>
    ...
    <div class="">
        <div class="grid grid-cols-1 p-2 rounded-lg">
            ...
            <div class="border-b dark:border-zinc-700 py-2 flex justify-between">
                ...
                @can('delete_penal_type_process')
                <div class="w-1/12 text-end">
                    <button wire:click="modalProcessPenealTypeDelete({{ $penal_type_process->id }})"><span
