<div>
  @can('create_documento_prison')
  <button wire:click="modalPrisonDocument({{ $prison->id }})"><span class="text-blue-600 mr-4">[ Adicionar
      ]</span></button>
  @endcan
  <div class="pt-4">
    ...
    <div class="grid grid-cols-2 px-2 rounded-lg gap-6">
      ...
      <div class="border-b dark:border-zinc-700 flex justify-between mx-6">
        <div class="text-sm uppercase font-medium">
          <a title='{{ $prison_document->title }}' href='{{ asset("storage/$prison_document->document") }}'
            rel='shadowbox[galeria]'>
            <dd class="font-semibold text-blue-700 dark:text-blue-500 hover:underline">{{
              $prison_document->title }}</dd>
          </a>
        </div>
        @can('admin-cartorio_admin')
        <div class="flex">
          @can('update_document_prison')
          <button wire:click="modalPrisonDocumentEdit({{ $prison_document->id }})"><span
              class="text-green-600 text-xs mr-4">[ Editar ]</span>
          </button>
          @endcan

          @can('delete_document_prison')
          <button wire:click="modalPrisonDocumentDelete({{ $prison_document->id }})"><span
