...
<div class="grid md:grid-cols-6 md:gap-6 mt-8">
    <div class="col-span-2 relative z-0 w-full group">
        ...
        <x-input-error for="requesting_id" class="mt-2">{{ $message ?? '' }}</x-input-error>
    </div>
...
<div class="grid md:grid-cols-4 md:gap-6 mt-8">
    <div class="col-span-1 relative z-0 w-full group">
        <x-input type="date" wire:model="event_date" id="event_date" />
        <x-label for="event_date" wire:model="event_date" value="{{ 'Data do Evento' }}" />
        <x-input-error for="event_date" class="mt-2">{{ $message ?? '' }}</x-input-error>
    </div>
    <div class="col-span-1 relative z-0 w-full group">
        <x-input type="text" wire:model="event_time" id="event_time" x-mask="99:99" />
        <x-label for="event_time" wire:model="event_time" value="{{ 'Hora do Evento' }}" />
        <x-input-error for="event_time" class="mt-2">{{ $message ?? '' }}</x-input-error>
    </div>
    <div class="col-span-1 relative z-0 w-full group">
    ...
    <div class="col-span-1 relative z-0 w-full group">
        <select id="municipality_id" wire:model="municipality_id" class="uppercase block py-1 mt-1 px-0 w-full text-sm text-zinc-500 dark:text-zinc-400 bg-transparent border-0 border-b border-zinc-400 dark:border-zinc-600 dark:focus:border-blue-500 focus:border-blue-600 appearance-none focus:outline-none focus:ring-0 peer">
            <option class="text-zinc-900 dark:text-zinc-600" selected value="{{ $municipalityEdit->id ?? '' }}">{{ $municipalityEdit->municipality ?? 'Município' }}</option>
...
<div class="grid md:grid-cols-4 md:gap-6 mt-8">
    <div class="col-span-1 relative z-0 w-full group">
        <x-input type="date" wire:model="departure_date" id="departure_date" />
        <x-label for="departure_date" wire:model="departure_date" value="{{ 'Data da Saída' }}" />
        <x-input-error for="departure_date" class="mt-2">{{ $message ?? '' }}</x-input-error>
    </div>
    <div class="col-span-1 relative z-0 w-full group">
    ...
    <div class="col-span-1 relative z-0 w-full group">
        <x-input type="date" wire:model="arrival_date" id="arrival_date" />
        <x-label for="arrival_date" wire:model="arrival_date" value="{{ 'Data do Retorno' }}" />
        <x-input-error for="arrival_date" class="mt-2">{{ $message ?? '' }}</x-input-error>
    </div>
    <div class="col-span-1 relative z-0 w-full group">
