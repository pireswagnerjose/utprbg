...
<div class="mx-auto p-4 relative border-b border-blue-400 dark:border-blue-600">
    ...
    <div class="flex z-10 absolute w-full items-center justify-end pr-6">
        ...
        {{-- Relatório PDF --}}
        <form action="{{ route('external-exit.report', ['external_exit_id' => $external_exit->id]) }}" method="POST"
            target="_blank">
            @csrf
            <div class="group grid justify-items-center w-16 border-b border-zinc-200 dark:border-zinc-600">
    ...
    <div class="grid grid-cols-6 gap-4 mb-5 items-center">
        ...
        <div class="col-span-2 text-sm uppercase font-medium flex justify-end">
            <a class="realtive flex items-center" title='{{ $external_exit->document }}'
                ...
                <dd class="font-semibold text-blue-700 dark:text-blue-500 hover:underline">DOCUMENTO RELACIONADO A SAÍDA
                </dd>
            </a>
            {{-- exclui o documento --}}
            <div class="group grid justify-items-center w-16">
                <button wire:click="modalDocumentDelete({{ $external_exit->id }})"
                    class="w-5 h-5 bg-red-600 dark:bg-red-500 rounded-full">
                    <svg class="w-5 h-5 text-red-50 dark:text-red-50 hover:text-red-400 hover:dark:text-red-400"
