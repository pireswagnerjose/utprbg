...
<div class="mx-auto p-4 relative border-b border-blue-400 dark:border-blue-600">
    ...
    <div class="flex z-10 absolute w-full space-x-8 items-center justify-end pr-6">
        @can('update_pad')
        <button wire:click="modalPadUpdate({{ $pad->id }})"
        ...
        @can('delete_pad')
        <button wire:click="modalPadDelete({{ $pad->id }})" wire:loading.attr="disabled"
    ...
    <div class="grid mb-5">
        ...
        <div class="px-4">
            <button wire:click="modalPadDocumentCreate({{ $pad->id }})"><span class="text-blue-600 mr-4">[ Adicionar
                    ]</span></button>
            <div class="">
                ...
                <div class="grid grid-cols-2 p-2 rounded-lg gap-20">
                    ...
                    <div class="border-b dark:border-zinc-700 flex justify-between">
                        <div class="text-sm uppercase font-medium">
                            <a title='{{ $pad_document->document }}'
                                href='{{ asset("storage/$pad_document->document") }}' rel='shadowbox[galeria]'>
                                <dd class="font-semibold text-blue-700 dark:text-blue-500 hover:underline">{{
                                    $pad_document->title }}</dd>
                            </a>
                        </div>
                        <div class="flex">
                            <button wire:click="modalPadDocumentUpdate({{ $pad_document->id }})"><span
                                    class="text-green-600 text-xs mr-4">[ Editar ]</span></button>
                            <button wire:click="modalPadDocumentDelete({{ $pad_document->id }})"><span
