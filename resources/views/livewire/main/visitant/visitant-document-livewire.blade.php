<div>
  @can('create_document_visitant')
  <button wire:click="modalVisitantDocumentCreate({{ $visitant_id }})"><span class="text-blue-600">[ Adicionar
      ]</span></button>
  @endcan
    ...
    <div class="grid grid-cols-2 p-2 rounded-lg gap-6">
      ...
      <div class="border-b dark:border-zinc-700 justify-between mx-6">
        <div class="text-sm uppercase font-medium">
          <a title='{{ $visitant_document->title }}' href='{{ asset("storage/$visitant_document->document") }}'
            rel='shadowbox[galeria]'>
            <dd class="font-semibold text-blue-700 dark:text-blue-500 hover:underline">{{
              $visitant_document->title }}</dd>
          </a>
