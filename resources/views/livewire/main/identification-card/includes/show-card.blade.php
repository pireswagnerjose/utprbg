        ...
        <div class="flex w-full gap-2">
                ...
                <div class="flex flex-row w-full">
                    <div class="w-[20%]">{{-- logo polícia penal --}}
                        <span class="flex flex-col w-full">
                            <img class="max-h-48 rounded-lg" src='{{ asset("storage/site/policia_penal_logo.png") }}' alt="{{ $identification_card->visitant->name }}">
                        </span>
                    </div>
                    <div class="w-[80%] text-center">{{-- cabeçalho --}}
                        ...
                        <span class="text-xs font-bold block">SUPERINTENDÊNCIA DE ADMINISTRAÇÃO <br>PENITENCIÁRIO E PRISIONAL</span>
            ...
            <div class="w-[25%]">{{-- foto do perfil --}}
                <span class="flex flex-col w-full h-full">
                    <img class="object-cover h-full rounded-lg" src='{{ asset("storage/" . $identification_card->visitant->photo ) }}' alt="{{ $identification_card->visitant->name }}">
                </span>
            </div>
        </div>
        <div class="mt-2 space-y-2">
            <span class="text-sm block">VISITANTE: <strong class="text-base">{{ $identification_card->visitant->name }}</strong></span>
            <span class="text-sm block">REEDUCANDO: <strong class="text-base">{{ $identification_card->prisoner->name }}</strong></span>
            <span class="text-sm block">PARENTESCO: <strong class="text-base">{{ $identification_card->degree_of_kinship->degree_of_kinship }}</strong></span>
            <div class="flex justify-between">
                <span class="text-base">CÓD.: <strong class="text-xl text-red-700">0000{{ $identification_card->id }} </strong></span>
                <span class="text-base">VALIDADE: <strong class="text-xl text-red-700">{{ \Carbon\Carbon::parse($identification_card->expiration_date)->format('d/m/Y') }}</strong></span>
            </div>
        </div>
    </div>
    <div class="border-2 border-zinc-600 dark:border-zinc-600 p-2 rounded-lg">
        <div class="mt-16 w-3/4 mx-auto border-t border-zinc-300 dark:border-zinc-600 text-center text-xs">ASSINATURA DO TITULAR OU RESPONSÁVEL</div>
        <div>
        ...
        <div class="mt-16 w-3/4 mx-auto border-t border-zinc-300 dark:border-zinc-600 text-center text-xs">DIRETOR DA UTPRBG</div>
    </div>
</div>
