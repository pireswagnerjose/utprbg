<div>
   <button wire:click="modalRelatedDocumentCreate({{ $assistance_with_public_defender->id }})" type="button"
...
<div>
    @forelse ($assistance_with_public_defender->assistance_with_public_defender_documents as $assistance_with_public_defender_document)
        <div class="grid grid-cols-2 px-2 rounded-lg gap-6">
            <div class="border-b dark:border-zinc-700 flex justify-between mx-6">
                <div class="text-sm uppercase font-medium">
                    <a title='{{ $assistance_with_public_defender_document->title }}' href='{{ asset("storage/$assistance_with_public_defender_document->path") }}' rel='shadowbox[galeria]'>
                        <dd class="font-semibold text-blue-700 dark:text-blue-500 hover:underline">{{ $assistance_with_public_defender_document->title }}</dd>
                    </a>
                </div>
                @can('admin-cartorio_admin')
                    <div class="flex">
                        <button wire:click="modalRelatedDocumentDelete({{ $assistance_with_public_defender_document->id }})"><span class="text-red-600 text-xs">[ excluir ]</span></button>
                    </div>
                @endcan
            </div>
        </div>
