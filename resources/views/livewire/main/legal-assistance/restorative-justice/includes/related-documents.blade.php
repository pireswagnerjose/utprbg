        ...
        <div class="grid grid-cols-2 px-2 rounded-lg gap-6">
            <div class="border-b dark:border-zinc-700 flex justify-between mx-6">
                <div class="text-sm uppercase font-medium">
                    <a title='{{ $restorative_justice_document->title }}' href='{{ asset("storage/$restorative_justice_document->path") }}' rel='shadowbox[galeria]'>
                        <dd class="font-semibold text-blue-700 dark:text-blue-500 hover:underline">{{ $restorative_justice_document->title }}</dd>
                    </a>
                </div>
                @can('admin-cartorio_admin')
                    <div class="flex">
                        <button wire:click="modalRelatedDocumentDelete({{ $restorative_justice_document->id }})"><span class="text-red-600 text-xs">[ excluir ]</span></button>
                    </div>
                @endcan
            </div>
        </div>
