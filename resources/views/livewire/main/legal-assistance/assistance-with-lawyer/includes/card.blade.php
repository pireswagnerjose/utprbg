<div class="mx-auto border-b border-blue-400 dark:border-blue-600">
    <div class="flex z-10 w-full justify-between my-2">
        @php
            $user_create = App\Models\User::where('id', $assistance_with_lawyer->user_create)->first();
            $user_update = App\Models\User::where('id', $assistance_with_lawyer->user_update)->first();
        @endphp
        ...
        <div class="flex items-center justify-end w-[30%] gap-2 sm:gap-4">
            @can('update_assistance_with_lawyers')
                <button wire:click="modalUpdate({{ $assistance_with_lawyer->id }}, 'lawyer')"
            ...
            @can('delete_assistance_with_lawyers')
                <button wire:click="modalDelete({{ $assistance_with_lawyer->id }})" wire:loading.attr="disabled"
    ...
    <div class="grid grid-cols-7 gap-4 mb-5">
        <div class="col-span-2">
            ...
            <x-item-data>{{ $assistance_with_lawyer->lawyer->lawyer }}</x-item-data>
        </div>

        <div class="col-span-1">
            <x-item-topic>Data </x-item-topic>
            <x-item-data>{{ \Carbon\Carbon::parse($assistance_with_lawyer->date_of_service)->format('d/m/Y') }}
            </x-item-data>
        </div>

        <div class="col-span-1">
            <x-item-topic>Hora </x-item-topic>
            <x-item-data>{{ $assistance_with_lawyer->time_of_service }}</x-item-data>
        </div>

        <div class="col-span-1">
            <x-item-topic>Status</x-item-topic>
            <x-item-data>{{ $assistance_with_lawyer->status }}</x-item-data>
        </div>

        <div class="col-span-1">
            <x-item-topic>Tipo do Atendimento</x-item-topic>
            <x-item-data>{{ $assistance_with_lawyer->modality_care->modality_care }}</x-item-data>
        </div>
    </div>
    ...
    <div>
        ...
        <x-item-data class="text-justify">{{ $assistance_with_lawyer->remark }}</x-item-data>
    </div>

    {{-- linha 3 --}}
    <div class="grid border-t mt-2 pt-2 border-zinc-300 dark:border-zinc-600">
        <div class="px-4">
            @include('livewire.main.legal-assistance.assistance-with-lawyer.includes.related-documents')
        </div>
    </div>
</div>
