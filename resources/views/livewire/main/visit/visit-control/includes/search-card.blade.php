        ...
        <tbody>
            @forelse ( $visit_controls as $key=>$visit_control )
                <tr class="odd:bg-white text-center text-base odd:dark:bg-zinc-900 even:bg-zinc-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700">
                    ...
                    <td class="p-2"> {{ $visit_control->visist_type }} </td>
                    <td class="p-2"> {{ $visit_control->ward->ward }} </td>
                    <td class="p-2">
                        <div class="flex w-full space-x-8 justify-center gap-8 ">
                            <button wire:click="modalUpdate({{ $visit_control->id }})" class="text-sm text-teal-500 font-semibold rounded hover:text-teal-800">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                </svg>
                            </button>
                            
                            <button wire:click="modalDelete({{ $visit_control->id }})" wire:loading.attr="disabled" class="text-sm text-red-500 font-semibold rounded hover:text-red-800">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
