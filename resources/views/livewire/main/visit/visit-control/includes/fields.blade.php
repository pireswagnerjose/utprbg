    ...
    <div class="grid md:grid-cols-2 md:gap-8 mb-8">
        <div class="col-span-1 z-0 w-full group">
            <select wire:model="visit_type"
                class="uppercase block py-1 px-0 w-full text-sm text-zinc-500 dark:text-zinc-400 bg-transparent border-0 border-b border-zinc-400 dark:border-zinc-600 dark:focus:border-blue-500 focus:border-blue-600 appearance-none focus:outline-none focus:ring-0 peer">
                <option class="text-zinc-900 dark:text-zinc-600 text-xs" selected value="{{ $visit_type ?? '' }}">{{
                    $visit_type ?? 'Tipo da Visita (social ou íntima)' }}</option>
                @foreach ($visit_types as $visit_type_value)
                <option class="text-zinc-900 dark:text-zinc-100 text-base" value="{{ $visit_type_value ?? '' }}"
        ...
        <div class="col-span-1 z-0 w-full group">
            <select wire:model="ward_id"
                class="uppercase block py-1 px-0 w-full text-sm text-zinc-500 dark:text-zinc-400 bg-transparent border-0 border-b border-zinc-400 dark:border-zinc-600 dark:focus:border-blue-500 focus:border-blue-600 appearance-none focus:outline-none focus:ring-0 peer">
                <option class="text-zinc-900 dark:text-zinc-600 text-xs" selected value="{{ $ward->id ?? '' }}">{{
                    $ward->id ?? 'Ala - Pavilhão' }}</option>
                @foreach ($wards as $ward)
                <option class="text-zinc-900 dark:text-zinc-100 text-base" value="{{ $ward->id ?? '' }}"
