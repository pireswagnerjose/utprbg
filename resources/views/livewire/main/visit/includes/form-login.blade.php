<div class="max-w-sm mx-auto p-4 bg-white border border-zinc-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-zinc-800 dark:border-zinc-700">
    <form class="space-y-8">
        ...
        <div class="col-span-1 z-0 w-full group py-8">
            <select wire:model="type" class="uppercase block py-1 px-0 w-full text-sm text-zinc-500 dark:text-zinc-400 bg-transparent border-0 border-b border-zinc-400 dark:border-zinc-600 dark:focus:border-blue-500 focus:border-blue-600 appearance-none focus:outline-none focus:ring-0 peer">
                <option class="text-zinc-900 dark:text-zinc-600 text-xs" selected value="">Escolha o Tipo de visita</option>
                @foreach ($this->visit_types as $visit_type_content)
                    <option class="text-zinc-900 dark:text-zinc-100 text-base" value="{{ $visit_type_content ?? '' }}" @selected(old('type') ==  $visit_type_content)>{{ $visit_type_content }}</option>
                @endforeach
            </select>
