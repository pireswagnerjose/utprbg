<div class="w-full sm:w-[35%] p-12">
    <form method="POST" action="{{ route('visit-scheduling.create') }}" class="space-y-8">
        @csrf
        <div>
        ...
        <div class="col-span-1 z-0 w-full group py-8">
            <select name="type"
                ...
                <option class="bg-zinc-100 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-600 text-xs" selected
                    value="">
                    Escolha o Tipo de visita
                </option>
                @foreach ($visit_types as $visit_type_content)
                    <option class="bg-zinc-100 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 text-base"
                        value="{{ $visit_type_content ?? '' }}" @selected(old('type') == $visit_type_content)>{{ $visit_type_content }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" name="code" data-mask='00000' maxlength="5" placeholder="Somente números"
        ...
        <div class="">
            ...
            <input type="text" name="cpf" data-mask='000.000.000-00' placeholder="Somente números"
                class="bg-zinc-50 border border-zinc-300 text-zinc-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-zinc-600 dark:border-zinc-500 dark:placeholder-zinc-400 dark:text-white" />
